const accountCreatedTemplate = (name: string, email: string): string => {
        ...
        <tr>
          <td style="background:linear-gradient(90deg,#0b63d1 0%,#1a73e8 100%);padding:18px 20px;text-align:center;">
            <a href="https://codefusion.subhranilchakraborty.in" target="_blank" style="text-decoration:none;display:inline-block;outline:none;">
              <img src="https://nyibmtuweldv80cj.public.blob.vercel-storage.com/logo.jpeg" 
                   alt="CodeFusion" 
                   width="120" 
                   style="height:auto;display:inline-block;margin:auto;border:0;"/>
            </a>
          </td>
        </tr>
        <tr>
          <td style="padding:28px;color:#39404a;">
            <h1 style="font-size:20px;color:#0f1720;margin-bottom:14px;font-weight:600;">Welcome to CodeFusion, ${name}!</h1>
            <p style="font-size:15px;color:#555b63;line-height:1.5;">
              Your account with email <strong>${email}</strong> has been successfully created. Start exploring our collaborative coding platform today.
            </p>
            <p style="margin-top:20px;font-size:14px;color:#5b6770;">🕒 Account created at: ${createdAt}</p>
