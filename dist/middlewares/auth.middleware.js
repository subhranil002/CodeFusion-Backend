...
const refreshAccessToken = async (req, res, next) => {
    try {
        ...
        try {
            decodedRefreshToken = jwt.verify(refreshToken, constants.REFRESH_TOKEN_SECRET);
        }
        catch (err) {
            throw new ApiError("Session expired! Please login again", 455);
        }
        ...
        return next(new ApiError(`auth.middleware :: refreshAccessToken: ${error.message || error}`, error.statusCode || 500));
    }
};
const isLoggedIn = async (req, res, next) => {
    try {
        ...
        try {
            const decodedAccessToken = jwt.verify(accessToken, constants.ACCESS_TOKEN_SECRET);
        ...
        return next(new ApiError(`auth.middleware :: isLoggedIn: ${error.message || error}`, error.statusCode || 500));
    }
};
