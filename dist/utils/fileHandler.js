...
const deleteLocalFiles = async () => {
    try {
        const tempDir = path.join(path.dirname(fileURLToPath(import.meta.url)), "../../public/temp");
...
const uploadImageToCloud = async (localFilePath) => {
    ...
    try {
        ...
        if (response?.moderation?.length > 0 &&
            response?.moderation[0]?.status === "rejected") {
            throw new ApiError("This image is not safe to upload, please upload a different image", 400);
        }
        return {
