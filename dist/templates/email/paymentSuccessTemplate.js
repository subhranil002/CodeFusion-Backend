...
const paymentSuccessTemplate = (email, planName, amount) => {
    const year = new Date().getFullYear();
    const paymentAt = format(toZonedTime(new Date(), "Asia/Kolkata"), "dd/MM/yyyy, hh:mm a");
    const billingPeriod = `${format(toZonedTime(new Date(), "Asia/Kolkata"), "dd/MM/yyyy")} - ${format(toZonedTime(addDays(new Date(), 30), "Asia/Kolkata"), "dd/MM/yyyy")}`;
